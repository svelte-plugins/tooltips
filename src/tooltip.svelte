<script>
	...
	onMount(() => {
		...
		if (tooltipRef !== null) {
			...
			const elementWidth = tooltipRef.getBoundingClientRect().width;
			const elementStyle = window.getComputedStyle(tooltipRef);
...
<span bind:this={containerRef} class="tooltip-container">
	<slot></slot>
	<div bind:this={tooltipRef} class="tooltip animation-{animation} {position} {theme}" class:arrowless={!arrow} style="min-width: {minWidth}px; max-width: {maxWidth}px; text-align: {align};">
		{#if !isComponent}
			{@html content}
		{/if}
	</div>
...
<style>
	...
	:root {
		...
		--tooltip-box-shadow: 0 1px 20px rgba(0,0,0,.25);
		--tooltip-font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen-Sans, Ubuntu, Cantarell, 'Helvetica Neue', sans-serif;
	...
	.tooltip.left.animation-bounce {
		...
		transform-origin: 50% 50%;
		transition: opacity .3s ease-in-out, transform .3s cubic-bezier(.5, -1, .5, 3);
	}
